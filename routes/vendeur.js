...
router.get('/',auth, (req, res) => {
    ...
    pool.query(`select (select B.quartier  from vendeur_day_zone as A join villes as B on A.idquartier=B.idquartier where A.day = ${day} and A.idvendeur=${idvendeur} ) as zone, (select A.nomprenom from users as A join resposablevendeur as B on A.id= B.idresponsable where B.idvendeur=${idvendeur}) as responsable, (select nomprenom from users where id=${idvendeur}) as vendeur, (select sum(pointtotal) from orders where idVendeur=${idvendeur} and (datecommande="${dateActuelle}")) as noteJour, (select count(id) from vendeurconsommateur where idvendeur=${idvendeur} and idquartier=(select idquartier from vendeur_day_zone where idvendeur=${idvendeur} and day=${day})) as nbrTotalConso, (select count(idConsommateur) from (SELECT DISTINCT idConsommateur FROM orders where idVendeur=${idvendeur} and datecommande="${dateActuelle}") as T) as consoValide, (select count(*) from (select idconsommateur from vendeurconsommateur where idvendeur=${idevendeur} and idquartier=(select idquartier from vendeur_day_zone where idvendeur=${idevendeur} and day=${day}) and idconsommateur not in (select DISTINCT idConsommateur from orders where idVendeur = ${idevendeur} and datecommande ="${dateActuelle}" and idConsommateur in (select id from users where quartier = (select idquartier from vendeur_day_zone where idvendeur=${idevendeur} and day=${day})))) as T) as consoAttente, ${consoRefuse}`, (err, result) => {
        if (err) {
